...
| `AUTH_KEY_DUPLICATED` / session used from different IPs | 该 session 文件已被 Telegram 作废。停止其他副本，重新登录生成只供当前服务使用的 session；不要在多个公网出口间复制同一个 session 文件。 |
| Hyperlinks/bold/italics lost | Update to the latest version — `msg.entities` is now passed through on copy. |

调度器检测到 `AuthKeyDuplicatedError`、`AuthKeyUnregisteredError` 或 `SessionRevokedError` 后，会保留 Web
管理与 `/healthz`，但停止后续 Telegram 调度，避免每个周期重复失败。健康响应的
`telegram_session_error` 会给出脱敏错误类型；安装新 session 后必须重启服务。
